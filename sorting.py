...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        mid = len(xs) // 2
        piv = xs[mid]
        less = [i for i in xs[1:] if cmp(i, piv) < 0]
        more = [i for i in xs[1:] if cmp(i, piv) >= 0]
        equal = [i for i in xs[1:] if cmp(i, piv) == 0]
...
def quick_sort(xs, cmp=cmp_standard):
    '''
    EXTRA CREDIT:
    The main advantage of quick_sort is that it`
    ...
    Even though quick sort and merge sort bo
    th have the same Theta(n log n) runtime,
    this more efficient memory usage typically makes quick sort faster in pr
    actice.
    (We say quick sort has a lower "constant factor" in its runtime.)
    The downside of implementing quick sort in this way is that it will no l
    onger be a [stable sort](https://en.wikipedia.org/wiki/Sorting_algorithm
    #Stability),
